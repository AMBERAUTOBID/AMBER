...
describe("mirrorPricing", () => {
  it("converts cents to dollars for a trustworthy currency", () => {
    expect(mirrorPricing(row())).toEqual({ current_bid_usd: 500, buy_now_usd: 1455 });
  });
  ...
  it("refuses to present an unknown currency as dollars", () => {
    ...
    expect(mirrorPricing(row({ currencyCode: "BRL" }))).toEqual({
      current_bid_usd: null,
      buy_now_usd: null,
    });
    expect(mirrorPricing(row({ currencyCode: "CAD" }))).toEqual({
      current_bid_usd: null,
      buy_now_usd: null,
    });
  });
  ...
  it("keeps an absent price absent rather than zero", () => {
    ...
    expect(mirrorPricing(row({ currentBidCents: null })).current_bid_usd).toBeNull();
    expect(mirrorPricing(row({ buyNowCents: null })).buy_now_usd).toBeNull();
  });
});
